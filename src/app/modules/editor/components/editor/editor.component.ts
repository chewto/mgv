...
export class EditorComponent {
  ...
  public onVoice(){
    ...
    this.speechDetectionSVC.text$.pipe(
      first()
    ).subscribe((res) => {
      console.log(res)
  ...
  private voiceTreatment(words:string): any{
    ...
    } else if(transpileWords[0] === 'estilo'){
      console.log('vas a usar css');
      finalCommand += this.speechTreatmentSVC.cssTranspile(transpileWords);
    } else {
      finalCommand += words;
    }
